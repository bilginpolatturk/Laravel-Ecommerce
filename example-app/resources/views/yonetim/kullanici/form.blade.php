...
@section('content')
    ...
    <form method="post" action="{{ route('yonetim.kullanici.kaydet',@$entry->id) }}">
        ...
        <h2 class="sub-header">Kullanıcı   {{ @$entry->id>0 ? "Düzenleme" : "Kayıt" }} Formu</h2>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="telefon" name="telefon" placeholder="Telefon Numarası" value="{{ old('adres',$entry->detay->telefon) }}">
                </div>
            </div>
        </div>
