...
class KullaniciController extends Controller
{
    ...
    public function index(){
        ...
        if (\request()->filled('aranan')){
            ...
            $list = Kullanici::orderByDesc('olusturulma_tarihi')->paginate(1);
        }
    ...
    public function kaydet($id = 0){
        ...
        if(\request()->has('aktif_mi') && \request('aktif_mi')==1){
            $data['aktif_mi'] = 1;
        }else{
            $data['aktif_mi'] = 0;
        }

        if(\request()->has('yonetici_mi') && \request('yonetici_mi')==1){
            $data['yonetici_mi'] = 1;
        }else{
            $data['yonetici_mi'] = 0;
        }
        ...
        return redirect()->route('yonetim.kullanici.duzenle',$entry->id)->with('mesaj_tur','success')->with('mesaj',($id>0 ? 'Güncellendi.' : 'Kaydedildi.'));
    ...
    public function sil($id){
        ...
        return redirect()->route('yonetim.kullanici')->with('mesaj_tur','success')->with('mesaj','Kullanıcı Silindi.');
    }
}
